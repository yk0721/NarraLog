    ...
    <div className="min-h-screen bg-gray-50 flex justify-center">
      <div className="w-full max-w-md bg-white shadow-xl min-h-screen flex flex-col">
        ...
        <header className="p-4 bg-white border-b sticky top-0 z-10 flex items-center justify-between">
          ...
          <div className="text-xs bg-gray-100 px-2 py-1 rounded">👤 {myName.current}</div>
        </header>
        ...
        {view === 'HOME' ? (
          <div className="p-4 flex flex-col gap-6 overflow-y-auto">
            ...
            <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
              ...
              <textarea
                className="w-full p-3 border rounded-lg text-sm mb-2 focus:outline-blue-400"
                rows={2}
            ...
            <div>
              ...
              <div className="space-y-3">
                {problems.map(p => (
                  <div key={p.id} className="border p-4 rounded-xl hover:shadow-md transition bg-white">
                    <h4 className="font-bold text-gray-800 mb-1">{p.title}</h4>
                    <p className="text-xs text-gray-500 mb-3 truncate">{p.content}</p>
                    <button
                      onClick={() => joinProblem(p)}
                      className="w-full border border-blue-600 text-blue-600 py-2 rounded-lg text-sm font-bold hover:bg-blue-50 flex items-center justify-center gap-2"
                    >
                      議論に参加する <ArrowRight size={14}/>
                    </button>
                  </div>
                ))}
                {problems.length === 0 && <p className="text-center text-gray-400 text-sm">まだ相談はありません</p>}
              </div>
            </div>
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50">
              {messages.map((m, i) => (
                <div key={i} className={`flex ${m.sender === myName.current ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] p-3 rounded-2xl text-sm shadow-sm ${
                    m.sender === myName.current
                      ? 'bg-blue-600 text-white rounded-br-none'
                      : 'bg-white text-gray-800 border rounded-bl-none'
                  }`}>
                    <div className={`text-[10px] mb-1 ${m.sender === myName.current ? 'text-blue-100' : 'text-gray-400'}`}>
                      {m.sender}
                    </div>
                    {m.content}
                  </div>
